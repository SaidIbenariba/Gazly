import { db } from "../connect_db.js";
  ...
  export const createMeeting = (req, res) => {
    const sql ="INSERT INTO meeting (`start`,`end`,`title`,`description`,`id_resp`,`id_Adir`,`allDay`) VALUE(?,?,?,?,?,?,?) ";
    const newTache = {
    ...
    export const getMeetings = (req, res) => {
      ...
      let 
      sql = `SELECT m.*, r.firstname, r.lastname FROM meeting m INNER JOIN users r ON m.id_resp = r.id WHERE  DATE(m.start) = CURDATE()`;
      if (userRole=='admin') {
        sql+="AND id_dir = ?";
    ...
    export const getAllMeetingsById = (req, res) => {
      let sql = "SELECT m.*, r.firstname, r.lastname FROM meeting m INNER JOIN users r ON m.id_resp = r.id ";
