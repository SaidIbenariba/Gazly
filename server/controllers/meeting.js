...
export const getMeetingsForAdmin = (req, res) => {
  ...
  export const createMeeting = (req, res) => {
    const sql ="INSERT INTO Meeting (date,`duree`,`description`,`id_resp`,`id_Admin`) VALUE(?,?,?,?,?) ";
    const newTache = {
      date : req.body.date,
    ...
    export const editMeeting= (req, res) => {
        const q =
          "UPDATE Meeting SET duree=?, description=?, id_Admin=?, id_resp=?  WHERE id_resp= ? ";
        const values = {
            Duree: req.body.duree,
            Description: req.body.description,
            id_Admin: req.body.id_Admin,
    ...
    export const Meetings = (req, res) => {
        const sql = "SELECT m.*,r.firstname,r.lastname FROM meeting m INNER JOIN users r ON  m.id_resp= r.id";
